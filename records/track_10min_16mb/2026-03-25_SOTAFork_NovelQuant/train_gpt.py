...
class Hyperparameters:
    ...
    grad_clip_norm = float(os.environ.get("GRAD_CLIP_NORM", 0.4))
        ...
        for group in self.param_groups:
            ...
            for i, m in enumerate(self._bank_meta):
                ...
                if prev_ag_handle is not None:
                    ...
                    if wd > 0.0:
                        if getattr(self, "_cautious_wd", False):
                            mask = (upd.sign() == pp.data.sign()).to(pp.dtype)
                ...
                if sharded:
                    ...
                    if wd > 0.0:
                        if getattr(self, "_cautious_wd", False):
                            mask = (update.sign() == p.data.sign()).to(p.dtype)
                            p.data.mul_(1.0 - lr * wd * mask)
                        else:
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim == 2:
        ...
        abs_sorted, _ = t32.abs().sort(dim=1)
        best_mse = torch.full((n_rows,), float("inf"), device=t32.device)
        best_scale = torch.zeros(n_rows, device=t32.device, dtype=torch.float16)
        best_q = torch.zeros_like(t32, dtype=torch.int8)
        for pct in [0.995, 0.999, 0.9995, 0.9999, 1.0]:
            k = min(int(pct * n_cols), n_cols - 1)
            clip = abs_sorted[:, k]
            sc = (clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
            qc = torch.clamp(torch.round(t32 / sc.float()[:, None]), -clip_range, clip_range).to(torch.int8)
            mse = ((qc.float() * sc.float()[:, None] - t32) ** 2).mean(dim=1)
            improved = mse < best_mse
            best_mse[improved] = mse[improved]; best_scale[improved] = sc[improved]; best_q[improved] = qc[improved]
...
def _get_layer_bits(name: str, adaptive_bits_str: str) -> int:
    if not adaptive_bits_str:
        return 6
    bits_list = [int(b) for b in adaptive_bits_str.split(",")]
    for i, b in enumerate(bits_list):
        if f"blocks.{i}." in name or f"[{i}]" in name:
            return b
    return 6

def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], adaptive_bits_str: str = ""):
...
def main() -> None:
    ...
    optimizer_muon._cautious_wd = args.cautious_wd
    ...
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, adaptive_bits_str=args.adaptive_quant_bits)
